...
STEPS = [
    ...
    {'num': 4, 'name': 'Rules',    'from': '{slug}-Unclassified.md',               'to': 'raw0/{first_level_dir}/Rules.md'},
    {'num': 5, 'name': 'Intent',   'from': '{slug}-Unclassified.md',               'to': 'raw0/{first_level_dir}/Intent.md'},
    {'num': 6, 'name': 'Tools-TodoList','from': '{slug}-Unclassified.md',          'to': 'raw0/{first_level_dir}/Tools-TodoList.md'},
    {'num': 7, 'name': 'Tools',    'from': '{slug}-Unclassified.md',               'to': 'raw0/{first_level_dir}/Tools.md'},
    {'num': 8, 'name': 'Emotional','from': '{slug}-Unclassified.md',               'to': 'raw0/{first_level_dir}/Emotional.md'},
    {'num': 9, 'name': 'Constraints-Confidential','from': '{slug}-Unclassified.md','to': 'raw0/{first_level_dir}/Constraints-Confidential.md'},
    {'num': 10,'name': 'Constraints','from': '{slug}-Unclassified.md',             'to': 'raw0/{first_level_dir}/Constraints.md'},
]
...
步骤 4：Rules → Rules.md
步骤 5：Intent → Intent.md
步骤 6：Tools-TodoList → Tools-TodoList.md
步骤 7：Tools → Tools.md
步骤 8：Emotional → Emotional.md
步骤 9：Constraints-Confidential → Constraints-Confidential.md
步骤 10：Constraints → Constraints.md"""
...
def build_prompt(step: dict, source: str, first_level_dir: str, slug: str) -> str:
    ...
    if step['num'] == 0:
        ...
        header = f"对源文件 `{from_path}` 执行 gosh-triage 分流过程，一共有步骤 0-10："
    else:
        ...
        header = f"对文件 `{unclassified_path}` 执行 gosh-triage 分流过程，一共有步骤 0-10："
