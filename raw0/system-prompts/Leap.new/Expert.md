<encore_ts_domain_knowledge>
  <general>
    ...
    The import path for all Encore.ts functionality starts from `encore.dev/`. Additionally, certain functionality is provided through auto-generated modules that are imported from `~encore/`, like `~encore/auth` for getting information about the authenticated user, and `~encore/clients` for making API calls between services.
...
export interface Cookie<Name extends string> {
  ...
  partitioned?: boolean;
    ...
    <streaming_api>
      ...
      - `streamInOut`: bidirectional streaming between client and client
