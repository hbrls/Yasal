...
Key API availability by requirement set:• 1.4+: body.getComments(), comment.reply(), range.insertBookmark(), document.changeTrackingMode• 1.5+: range.insertFootnote(), range.insertField(), body.fields.getByTypes(), field.updateResult(), document.insertFileFromBase64() with import options• 1.6+: body.getTrackedChanges(), paragraph.uniqueLocalId
